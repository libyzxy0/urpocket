...
private:
  ...
  const char keyboard[32] = "ABCDEFGHIJKLMNO" "PQRSTUVWXYZ <->";
  const int totalKeys = 31;
  ...
  void run(Adafruit_SH1106G &display, AppNetworkManager &net, AudioSystem &audio, bool up, bool down, bool select, bool selectDoubleClicked) {
    ...
    if (keyboardActive) {
      ...
      for (int i = 0; i < totalKeys; i++) {
        ...
        int xPos = col * keyWidth + 4;
        int yPos = startY + (row * (keyHeight + 2));

        if (i == currentKeyIndex) {
          display.fillRect(xPos - 1, yPos - 1, keyWidth, keyHeight, SH110X_WHITE);
          display.setTextColor(SH110X_BLACK);
