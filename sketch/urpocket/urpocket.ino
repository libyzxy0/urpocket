...
void drawOTAUIPage(bool updating, int progressPercent, const char* statusMsg, IPAddress ip) { 
  display.clearDisplay(); 
  display.setTextColor(SH110X_WHITE); 
  display.setTextSize(1); 
...
void setupOTAHandlers() { 
  ArduinoOTA.onStart([=]() { 
    drawOTAUIPage(true, 0, "Updating Firmware...", WiFi.localIP()); 
  }); 
  
  ArduinoOTA.setPassword("libyzxy0"); 
  
  ArduinoOTA.onEnd([=]() { 
    drawOTAUIPage(false, 100, "Update Complete!", WiFi.localIP()); 
    delay(1000);
  }); 
  
  ArduinoOTA.onProgress([=](unsigned int progress, unsigned int total) { 
    int pct = (total > 0) ? (progress * 100UL / total) : 0;
    drawOTAUIPage(true, pct, "Updating Firmware...", WiFi.localIP()); 
  }); 
  ...
  ArduinoOTA.onError([=](ota_error_t error) { 
    ...
    drawOTAUIPage(false, 0, msg, WiFi.localIP()); 
  }); 
}
...
void turnOffDevice() {
  ...
  esp_deep_sleep_enable_gpio_wakeup(1ULL << BUTTON_SELECT, ESP_GPIO_WAKEUP_GPIO_LOW);
  esp_deep_sleep_start();
}


void loop() {
  if (currentState == STATE_SETTINGS && currentSettingsSubState == SETTINGS_OTA && wifiEnabled) {
    ArduinoOTA.handle();
  }
...
void runChat(bool up, bool down, bool select) {
  ...
  if (sendingStatus) {
    ...
    display.setTextSize(1);
    display.setTextColor(SH110X_WHITE);
    display.print("Proccessing please wait...");
    display.display();
    return;
  }
...
void runSettings(bool up, bool down, bool select) {
  if (currentSettingsSubState == SETTINGS_MAIN) {
    ...
    display.setCursor(0, 0);
    display.print("WIFI");
    display.setCursor(wifiEnabled ? 85  : 80, 0);
    ...
    startOTA();
    drawOTAUIPage(false, 0, "OTA is Ready", WiFi.localIP());
...
void runPowerOff(bool up, bool down, bool select) {
  ...
  if (powerMenuSelection == 1) {
    display.fillRoundRect(68, 4, 58, 56, 4, SH110X_WHITE);

    display.drawBitmap(84, 8, image_device_reset_bits, 26, 32, SH110X_BLACK);
    display.setTextColor(SH110X_BLACK);
  } else {
    display.drawBitmap(84, 8, image_device_reset_bits, 26, 32, SH110X_WHITE);
    display.setTextColor(SH110X_WHITE);
  }
